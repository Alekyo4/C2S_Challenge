...
from google.generativeai.types import AsyncGenerateContentResponse, GenerationConfigType, ContentsType
...
class GeminiAgentAI(AgentAIProvider):
  ...
  def __to_genai_history(self, history: list[ChatMessageDto]) -> ContentsType:
    return [{ "role": "model" if msg.role == "assistant" else "user", "parts": [ { "text": msg.content } ]} for msg in history]

  async def get_chat_response(self, history: list[ChatMessageDto], tools: list[dict[str, any]] | None = None) -> LLMResponse:
    response: AsyncGenerateContentResponse = await self.model.generate_content_async(
  ...
  async def extract_structured(self, history: list[ChatMessageDto], schema: type[BaseModel]) -> LLMResponse:
    ...
    response: AsyncGenerateContentResponse = await self.model.generate_content_async(
      contents=self.__to_genai_history(history), generation_config=genai_config)
    
    validate: BaseModel = schema.model_validate_json(response.text)
    
    return LLMResponse(
      tool_arguments=validate.model_dump(exclude_none=True),
      is_tool_call=True)