...
from c2s_challenge.common.protocol.dto import VehicleFilterDto, VehicleChatIDto, VehicleChatODto
...
class VehicleChatHandler(EventHandler):
  ...
  async def handle(self, data: VehicleChatIDto) -> Response:
    res_chat: LLMResponse = await self.agent_ai.get_chat_response(data.history, tools=[{
    ...
    if not res_chat.is_tool_call:
      result: VehicleChatODto = VehicleChatODto(finished=False, content=res_chat.text)

      return Response(status="success", data=result)
    
    res_extract: LLMResponse = await self.agent_ai.extract_structured(data.history, VehicleFilterDto)

    result: VehicleChatODto = VehicleChatODto(finished=True, content=res_extract.tool_arguments)
